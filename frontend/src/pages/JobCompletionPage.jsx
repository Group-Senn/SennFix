...
function JobCompletionPage() {
  ...
  useEffect(() => {
    const verifyClient = async () => {
      ...
      try {
        const response = await fetch(`${window.API_URL}/api/jobs/${jobId}`, {
  ...
  const handleConfirmSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${window.API_URL}/api/jobs/${jobId}/client-confirm`, {
        method: 'POST',
        headers: { 
  ...
  const handleRequestMediation = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${window.API_URL}/api/jobs/${jobId}/request-mediation`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
